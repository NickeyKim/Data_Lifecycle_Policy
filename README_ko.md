...
**Lakeview 대시보드** (`Data Lifecycle Policy Dashboard`)가 번들에 포함되어 함께 배포됩니다. 다음을 시각화합니다:

- 오래된 테이블 스캔 결과 및 후보 건수
- 작업 실행 상태 (DRY_RUN / TRUNCATE / SKIP)
- 과거 라이프사이클 정책 실행 추이

대시보드 정의 파일은 `src/data_lifecycle_policy_dashboard.lvdash.json`에 저장되어 있으며, `resources/data_lifecycle_policy_dashboard.yml`에서 리소스로 설정됩니다.
...
│   └── data_lifecycle_policy_dashboard.yml     # Lakeview 대시보드 리소스
├── src/
│   ├── Data Lifecycle Policy.py                # 메인 라이프사이클 정책 노트북
│   └── data_lifecycle_policy_dashboard.lvdash.json  # Lakeview 대시보드 정의
└── scratch/
